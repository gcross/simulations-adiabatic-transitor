-- @+leo-ver=4-thin
-- @+node:gcross.20091215183613.1295:@thin plot-adiabatic-random-angle-gap.hs
-- @@language Haskell
...
main =
    ...
    >>=
    flip withSession (
        let sql_statement_1 = "select number_of_sites, energy_gap from adiabatic_random_angle_simulations order by number_of_sites asc;"
            sql_statement_2 = "select number_of_sites, min(energy_gap) from adiabatic_random_angle_simulations group by number_of_sites order by number_of_sites asc;"
        in do
